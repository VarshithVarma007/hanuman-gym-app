...
# Determine user permission matrices for conditional tab mapping
is_owner = (st.session_state.active_username == "varshith")

# Build Tab Horizon dynamic matrix array layout
...
with tabs_list[5]:
    ...
    if st.button("Sync Metrics Arrays to Local CSV Disk File Layer", use_container_width=True, key="save_m"):
        ...
        new_df = pd.DataFrame([{"Timestamp": timestamp_now, "Username": st.session_state.active_username, "Bicep (in)": bi, "Chest (in)": ch, "Waist (in)": wa, "Body Fat (%)": bf}])
        new_df.to_csv(ledger_file, mode='a', header=False, index=False)
...
with tabs_list[6]:
    ...
    if plan_col1.button("💪 Starter Pass\n\n ₹1,500 / Month", key="p_m1", use_container_width=True):
        pd.DataFrame([{"Timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "Username": st.session_state.active_username, "Plan": "Starter Pass", "Amount": 1500}]).to_csv(payments_file, mode='a', header=False, index=False)
        st.success("🎉 Plan configuration registered successfully!")
    if plan_col2.button("🔥 Quarterly Pack\n\n ₹4,000 / 3 Months", key="p_m2", use_container_width=True):
        pd.DataFrame([{"Timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "Username": st.session_state.active_username, "Plan": "Quarterly Pack", "Amount": 4000}]).to_csv(payments_file, mode='a', header=False, index=False)
        st.success("🎉 Plan configuration registered successfully!")
    if plan_col3.button("👑 Iron Elite VIP\n\n ₹12,000 / Year", key="p_m3", use_container_width=True):
        pd.DataFrame([{"Timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "Username": st.session_state.active_username, "Plan": "Iron Elite VIP", "Amount": 12000}]).to_csv(payments_file, mode='a', header=False, index=False)
        st.success("🎉 Plan configuration registered successfully!")
...
# -------------------------------------------------------------
# TAB 8: 👑 ADMIN HEADQUARTERS (CONDITIONAL ROLE TAB FOR OWNER)
# -------------------------------------------------------------
if is_owner:
    with tabs_list[7]:
        ...
        st.write("Secure business reporting and system monitoring dashboard interface.")
        ...
        if os.path.exists(payments_file):
            ...
            st.warning("No payments tracking files generated on active server matrices yet.")
            
        st.write("---")
        
        # 📚 MEMBER TELEMETRY DRILL DOWN MODULE
        st.markdown("### 📚 Centralized Member Telemetry Ledger")
        try:
            users_df = pd.read_csv(users_file)
            registered_members = users_df['username'].unique().tolist()
            
            selected_member_node = st.selectbox("Select Target Active Member Accounts for Audit Profile View:", registered_members)
            
            if os.path.exists(ledger_file):
                global_ledger_df = pd.read_csv(ledger_file)
                # Filter down historical inputs mapping exclusively for selected user account
                filtered_member_data = global_ledger_df[global_ledger_df['Username'] == selected_member_node]
                
                if not filtered_member_data.empty:
                    st.success(f"Displaying historical physical diagnostic datasets for member reference: `{selected_member_node.upper()}`")
                    st.dataframe(filtered_member_data, use_container_width=True)
                else:
                    st.info(f"Member account identity node tracking verified: `{selected_member_node}`, but no biometric measurement data points recorded on disk logs yet.")
            else:
                st.error("Operational baseline ledger system file not discovered.")
        except Exception as err:
            st.error(f"Failed to audit user base registries. Traceback exception: {err}")
